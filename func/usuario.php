...
function iniciarVaraibleSessionUser($usuario){
...
function busquedaPersonas($usuario, $persona, $pagina){
        ...
        if(strlen($persona)>2){
            ...
            if($usuario!=null){
                ...
                $consulta = $pdo->prepare("SELECT usuario FROM web.db_usuarios WHERE"
                    . " (usuario LIKE :persona)  AND (activo=1)"
                    . "  LIMIT :inicio, :numres");
...
function getFechaAmistad($usuario, $amigo){
    
    
    global $pdo;
    $consulta = $pdo->prepare("SELECT fecha_aceptado FROM web.db_amigos WHERE"
            . " usuario = :usuario AND amigo = :amigo ");
    $consulta->bindParam(":usuario", $usuario);
    $consulta->bindParam(":amigo", $amigo);
    $consulta->execute();
    ...
    return $resultado["fecha_aceptado"];
...
function editarEmailUsuario ($usuario, $email){
    global $pdo;
...
function editarEstadoUsuario ($usuario, $estado){
    global $pdo;
