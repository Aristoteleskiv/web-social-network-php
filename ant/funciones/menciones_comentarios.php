...
function sustituirCitas($cuerpo, $idPublicacion, $posicionSiguiente){
...
function sustituirMenciones($idPublicacion, $cuerpo, $posicionSiguiente){
    ...
    for($i=0; $i<count($menciones[0]); $i++){
        ...
        if(comprobarSiUsuarioExisteParaMencion($usuario) ){   
            
            $aReemplazar = '<span class="mencion_usuario_no_enlace">@' . $usuario.  '</span> ';
...
function comprobarSiUsuarioExisteParaMencion($usuario){
    global $pdo;
...
function getComentariosPorRelevancia($id_publicacion){
    ...
    $consulta = $pdo->prepare("SELECT * FROM web.db_comentarios WHERE "
            . " id_publicacion = :id_publicacion ORDER BY "
